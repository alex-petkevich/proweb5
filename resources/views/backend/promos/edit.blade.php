...
@section('main')
    ...
    <div class="col-xs-10">
        ...
        @if ($post->id)
            <h1 class="post-header">{!! trans('promos.edit_post') !!}</h1>
            {!! Form::model($post, array('method' => 'PATCH', 'route' => array('posts.update', $post->id), 'post' => 'form')) !!}
        @else
            <h1 class="post-header">{!! trans('promos.create_post') !!}</h1>
        ...
        <div class="form-group @if ($errors->has('category')) has-error has-feedback @endif">
            ...
            {!! Form::text('categories', Input::old('categories', implode(', ', array_pluck($post->categories()->get(array('title'))->toArray(), 'title'))),array('class' => 'form-control', 'id' => 'categories-view', 'readonly'=>'1')) !!}
            {!! Form::hidden('categories_ids',implode(',', array_pluck($post->categories()->get(array('blog_category_id'))->toArray(), 'blog_category_id')), array('id'=>'categories-hidden')) !!}
        </div>
        ...
        <div class="form-group @if ($errors->has('title')) has-error has-feedback @endif">
            {!! Form::label('title', trans('promos.post'), array('class' => 'control-label')) !!}
        ...
        <div id="additional_options" style="display:none;">
            ...
            <div class="form-group @if ($errors->has('meta_description')) has-error has-feedback @endif">
                {!! Form::label('meta_description', trans('promos.meta_description'), array('class' => 'control-label')) !!}
            ...
            <div class="form-group @if ($errors->has('meta_keywords')) has-error has-feedback @endif">
                {!! Form::label('meta_keywords', trans('promos.meta_keywords'), array('class' => 'control-label')) !!}
            ...
            <div class="form-group @if ($errors->has('only_for_roles')) has-error has-feedback @endif">
                {!! Form::label('only_for_roles', trans('promos.only_for_roles'), array('class' => 'control-label')) !!}
            ...
            <div class="form-group @if ($errors->has('file')) has-error has-feedback @endif">
                {!! Form::label('file', trans('promos.avatar'), array('class' => 'control-label')) !!}
                {!! Form::file('file') !!}
        ...
        <div class="form-group @if ($errors->has('publishied_at')) has-error has-feedback @endif">
            {!! Form::label('publishied_at', trans('promos.published_at'), array('class' => 'control-label')) !!}

            <div class='input-group date' id='publishpicker'>
                {!! Form::text('publishied_at',$post->published_at, array('class' => 'form-control')) !!}
        ...
        <div class="form-group">
            {!! Form::submit(trans('promos.submit_draft'), array('class' => 'btn btn-info', 'id' => 'draft')) !!}
            {!! Form::submit(trans('promos.submit_publish'), array('class' => 'btn btn-danger', 'id' => 'publish')) !!}
            {!! link_to_route('posts.index', trans('promos.cancel'), null, array('class' => 'btn btn-default')) !!}
        </div>
