   ...
   <div class="modal-dialog" role="document">
      <div class="modal-content">
         <div class="modal-header">
            ...
            <h4 class="modal-title" id="myModalLabel">{!! trans('blog_categories.select_cats') !!}
            </h4>
         </div>
               ...
               <div class="row">
                  ...
                  <div class="col-md-6">
                     <button type="button" class="btn btn-primary"
                             id="selCat">{!! trans('blog_categories.submit') !!}</button>
...
<script type="text/javascript">
   ...
   function getTree() {
      $('#progress').show();
      $.ajax({
         url: '/api/blog/categories?active=1',
   ...
   function displayValue(key, val, element, shift) {
      ...
      current += "<label for='cat-" + val.id + "'>" + (val.active != '1' ? '<s>' : '') + val.title + (val.active != '1' ? '</s>' : '') + "</label>";
      current += "</div><div class=\"col-md-4 text-right\">";
      current += "<input type='checkbox' name='cat_id' id='cat-" + val.id + "' value='" + val.id + "' />";
      current += "</div></div></div>";
      current += "</li>";
